...
//! `linux/input-event-codes.h`), and mapping a character to the keycode-plus-shift that
//! produces it under the US layout is a fixed table. This is display-independent and
//! unit-tested directly.
//!
//! This table is NO LONGER the primary path for `type <text>`: the seat advertises the HOST
//! layout, so `input::type_text` resolves each character through `layout_keymap`, which
//! inverts the very keymap the client was handed (finding D7). This module stays for two
//! jobs: the FALLBACK when a character is not reachable on that layout with at most Shift,
//! and `named_key`, which the `key <name>` command uses to press keys by name.
...
pub fn named_key(name: &str) -> Option<u32> {
    ...
    match name {
        ...
        "pageup" => Some(104),
        "pagedown" => Some(109),
        // Modifiers, so a control-socket caller can build chords by hand:
        // `key ctrl press` / `key v tap` / `key ctrl release`. These are the LEFT-hand
        // evdev codes (KEY_LEFTCTRL 29, KEY_LEFTSHIFT 42, KEY_LEFTALT 56, KEY_LEFTMETA
        // 125); a chord needs one side only.
        "ctrl" | "control" | "leftctrl" => Some(29),
        "shift" | "leftshift" => Some(LEFT_SHIFT),
        "alt" | "leftalt" => Some(56),

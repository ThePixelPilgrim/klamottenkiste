...
///
/// What:     `impl SelectionHandler for Compositor`. The associated type `()` is the unit
///           type (an empty tuple, "no data"), meaning we attach no extra data to the
///           server-side selections we set.
/// Why:      This is where the SPIKE-scope clipboard bridge hooks in: `new_selection`
///           fires when the hosted client copies, `send_selection` fires when it pastes
///           from a selection the compositor owns. Both delegate to `crate::clipboard`,
///           which carries the text to and from the host's clipboard. Everything except
///           the clipboard target (i.e. primary selection) is deliberately ignored.
impl SelectionHandler for Compositor {
    /// No extra per-selection data.
    type SelectionUserData = ();

    /// What:     `fn new_selection(&mut self, ty, source, _seat)`. The hosted client set a
    ///           selection; `source` is `None` when it cleared it.
    /// Why:      Record which text mime type to read back. The read itself is deferred to
    ///           `clipboard::service` because Smithay calls this BEFORE it stores the new
    ///           selection on the seat.
    fn new_selection(
        &mut self,
        ty: SelectionTarget,
        ...
        crate::clipboard::note_client_selection(self, source.as_ref());
    }

    /// What:     `fn send_selection(&mut self, ty, mime_type, fd, _seat, _user_data)`. The
    ///           hosted client is pasting from a compositor-owned selection and wants the
    ///           bytes for `mime_type` written into `fd`.
    /// Why:      Serve the host clipboard text the bridge last stored.
    fn send_selection(
        &mut self,
        ty: SelectionTarget,
        mime_type: String,
        ...
        crate::clipboard::send_host_text(self, &mime_type, fd);
    }
}

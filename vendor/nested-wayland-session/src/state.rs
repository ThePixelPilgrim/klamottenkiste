...
/// What:     `use crossbeam_channel::{Receiver, Sender};`. The cross-thread queue halves.
/// Why:      Real GTK input is enqueued by the host thread (`Sender`) and drained inside
///           the compositor loop (`Receiver`); the state owns both ends so the loop can
///           read the receiver and hosts can clone the sender.
use crossbeam_channel::{Receiver, Sender};

/// What:     `use crate::input::SpikeInput;`. The GTK-sourced input event enum.
/// Why:      The state's input channel carries these events to the seat.
...
pub struct Compositor {
    ...
    /// The render backend: the headless GLES renderer + offscreen buffer (or, with
    /// `backend_winit`, the winit graphics backend).
    ///
    /// What:     `pub backend: crate::backend::RenderBackend`. A type alias resolving to
    ///           `HeadlessBackend` by default.
    /// Why:      Rendering, dmabuf import, and screenshot readback all go through it.
    pub backend: crate::backend::RenderBackend,
    ...
    /// A cloneable handle to the event loop, for registering timer sources.
    ///
    ...
    /// Why:      Control handlers (which only have `&mut Compositor`) can insert sources.
    ...
    /// The most recently composited frame, read back to CPU memory for the GTK host.
    ///
    /// What:     `pub latest_frame: Arc<Mutex<Option<crate::app::Frame>>>`. A thread-safe
    ///           slot holding the latest upright RGBA frame (or `None` before the first).
    /// Why:      The redraw timer (compositor thread) publishes each frame here; the GTK
    ///           host thread reads it out via `HeadlessHandle::latest_frame`. Only the
    ...
    ///           its event controllers can enqueue pointer/keyboard/scroll/focus events.
    /// Why:      Held in state so `spawn_headless` can clone it out for the host after the
    ///           state is built (the receiver stays here and is drained by the loop).
    pub input_tx: Sender<SpikeInput>,

    /// Receiving half of the real-GTK-input queue.
    ///
    /// What:     `pub input_rx: Receiver<SpikeInput>`. Drained once per event-loop
    ///           iteration by `input::drain_input`, which applies each event to the seat.
    /// Why:      Injecting input must happen on the compositor thread where the seat
    ///           lives; the queue carries events across from the GTK thread.
    pub input_rx: Receiver<SpikeInput>,

    /// SPIKE SCOPE: the text-only clipboard bridge to the host clipboard.
    ///
    /// What:     `pub clipboard: crate::clipboard::ClipboardBridge`. Holds the two
    ///           compositor<->host text channels plus the currently offered host text and
    ///           the loop-guard bookkeeping.
    /// Why:      The `SelectionHandler` callbacks only get `&mut Compositor`, so the
    ///           bridge's state has to live here. See `crate::clipboard` for the thread
    ///           model and the deliberate limitations.
    pub clipboard: crate::clipboard::ClipboardBridge,
}
...
pub struct BackendPieces {
    /// The render backend (headless GLES renderer + offscreen buffer by default).
    pub backend: crate::backend::RenderBackend,
    /// The nested output.
    pub output: Output,
...
impl Compositor {
    ...
    pub fn new(
        ...
        // What:     `let mut seat: Seat<Self> = seat_state.new_wl_seat(&dh, "seat0");`.
        ...
        // What:     Resolve the HOST keyboard layout and hand it to the seat, instead of
        //           `Default::default()` (which is a US layout). `200, 25` are the key
        //           repeat delay (ms) and rate (keys/s).
        // Why:      The nested seat gives the client its keymap. Advertising US while the
        //           user types on e.g. a `de` keyboard mistranslates every differing key
        //           (y/z, umlauts, most symbols). See `xkb_host` for the lookup order.
        //           Falls back to the default config if the host layout is unknown or the
        //           resolved keymap fails to compile, so an odd layout can never stop the
        //           compositor from starting.
        let host_xkb = crate::xkb_host::resolve();
        let keyboard_added = if host_xkb.is_empty() {
            seat.add_keyboard(Default::default(), 200, 25)
        } else {
            let config = XkbConfig {
                rules: &host_xkb.rules,
                model: &host_xkb.model,
                layout: &host_xkb.layout,
                variant: &host_xkb.variant,
                options: if host_xkb.options.is_empty() {
                    None
                } else {
                    Some(host_xkb.options.clone())
                },
            };
            seat.add_keyboard(config, 200, 25).or_else(|err| {
                tracing::warn!(
                    "host xkb layout {:?} failed to compile ({err:#}); falling back to default",
                    host_xkb.layout
                );
                seat.add_keyboard(Default::default(), 200, 25)
            })
        };
        keyboard_added.unwrap();
        ...
        // What:     `let (input_tx, input_rx) = crossbeam_channel::unbounded();`. Create the
        //           real-GTK-input queue: unbounded so a burst of motion events never blocks
        //           the GTK main thread.
        // Why:      The host clones `input_tx` to enqueue events; the loop drains `input_rx`.
        let (input_tx, input_rx) = crossbeam_channel::unbounded::<SpikeInput>();

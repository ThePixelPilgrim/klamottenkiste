...
/// What:     `use std::{process::Command, time::Duration};`. The child process builder
///           and the exit-poll interval.
/// Why:      `spawn_child` builds a `Command` directly; the timer needs `Duration`.
///
...
/// Why:      `spawn_child` reads/writes state.
///
/// In TS you'd write (pseudocode):
/// ```ts
...
pub fn spawn_child(state: &mut Compositor, command: &[String]) -> Result<()> {
    ...
    // What:     `let mut cmd = Command::new(program); cmd.args(&command[1..]);`. Build the
    //           spawn command directly (the systemd CPU-isolation wrapper was removed with
    //           the headless refactor). `mut` because we set the Wayland environment next.
    // Why:      Launch the app straight, no resource-control scope.

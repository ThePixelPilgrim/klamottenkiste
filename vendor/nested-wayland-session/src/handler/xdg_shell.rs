...
/// Configure one toplevel to fill the output: set maximized + activated + the
/// output size as its pending size.
///
/// What:     `fn set_maximized(surface: &ToplevelSurface, output: &Output)`. Private
...
/// ```
fn set_maximized(surface: &ToplevelSurface, output: &Output) {
    ...
    // Why:      Stage the maximized flags and size so the next `send_configure` carries
    //           them to the client.
    //
    // In TS you'd write (pseudocode):
    // ```ts
    ...
    surface.with_pending_state(|state| {
        // What:     `state.states.set(xdg_toplevel::State::Maximized);`. Adds the
        //           maximized flag to the toplevel's state set.
        // Why:      Make the app fill the pane while keeping its NORMAL window UI.
        //           This was `Fullscreen`, which a test fixture can get away with but a
        //           real browser honours semantically: Chrome treated every configure as
        //           F11 and hid its tab strip and omnibox, so each pane resize threw it
        //           into fullscreen unasked. `Maximized` means "fill the space available"
        //           without the kiosk semantics — which is what an embedded pane wants.
...
///
/// What:     `pub fn reconfigure_maximized(state: &mut Compositor)`. Iterates the
///           space's windows, re-applies `set_maximized`, and sends a fresh configure.
...
/// ```
pub fn reconfigure_maximized(state: &mut Compositor) {
    ...
    //           `set_maximized` (which borrows `state.output`), avoiding an
    ...
    for toplevel in &toplevels {
        // What:     `set_maximized(toplevel, &state.output);`. Stage the new size.
        // Why:      Update the pending configure to the new output size.
        set_maximized(toplevel, &state.output);

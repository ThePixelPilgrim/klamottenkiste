...
#[test]
fn modifier_names_resolve_to_the_left_hand_codes() {
    // Needed so a control-socket caller can build chords: `key ctrl press`, `key v tap`,
    // `key ctrl release`.
    assert_eq!(named_key("ctrl"), Some(29));
    assert_eq!(named_key("control"), Some(29));
    assert_eq!(named_key("shift"), Some(LEFT_SHIFT));
    assert_eq!(named_key("alt"), Some(56));
    assert_eq!(named_key("super"), Some(125));
}
